  ...
  class admin extends sql_commands
   {
	...
	function edit_user($email,$pass,$rpass,$cell)
	{
	
		$this->db=Connect::connect();
		$query="update register set password='".$pass."', Rpassword='".$rpass."', cellN = '".$cell."' where emai1='$email' LIMIT 1 ";
		$result=$this->db->query($query);
		$msg=mysqli_insert_id($this->db);	
	...
	function select_for_components()
	{
		$ad_products="";
		$id="";
		$prod="";
		...
		if($num_row>0)
		{
			for($a=0; $a<$num_row; $a++)
			{
				$rows=$result->fetch_assoc();
					...
					$details=$rows['pro_details'];
					$cat=$rows['pro_cat'];
									$ad_products.='<table border="0" cellspacing="0" cellpadding="6">
	<tr>			
		...
		<td valign="top">'."Name: ".$prod.'<br/> '."Price: ".'R'.$price.'<br/>'."Details: ".$details.'<br/>
